...
class FriendshipService(object):
    ...
    def get_followers(cls, user):
        ...
        # from_user是一个实例，是一个user class的实例
        # 因为是一个实例，所以它需要手拿着from_user_id去数据库里找对应的记录行
        # from_user_id是friendship表单中的一个字段，是一个int
    ...
    # 如果数据库出现了更新，一般会让缓存失效而不是去更新缓存
    # 如果去更新cache的话，那么在高并发的场景下有可能会导致数据的不一致
    @classmethod
